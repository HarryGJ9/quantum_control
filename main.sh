...
if [[ "$user_input" == "y" ]]; then
    ...
    do
        ...
        if [ -d /home/hgjones9/quantum_control/output_genome_$i ]; then
            rm -rf /home/hgjones9/quantum_control/output_genome_$i
        fi

        # Change name of output folder to reference later
        mv /home/hgjones9/quantum_control/output-latest /home/hgjones9/quantum_control/output_genome_$i

        # Obtain gen optimised genome and backup incase first iteration doesn't improve
        python3 /home/hgjones9/quantum_control/genetic_genome.py "$user_input" "$i"
        cp new_genome_$i.txt new_genome_backup_$i.txt

        # Retrieve max fidelity and corresponding time of gen optimised genome
        python3 /home/hgjones9/quantum_control/max_fidelity_time.py "$user_input" "$i"

    done

    # Specify <i|f> directive
    genetic_out_file='/home/hgjones9/quantum_control/output_genome_1/genetic.out' # Path to genetic.out
    ...
    do
        ...
        old_fidelity=$(head -n 1 /home/hgjones9/quantum_control/max_fidelity_time_$i.txt) 
        infidelity=$(echo "100 - $old_fidelity" | bc) 

        echo "Initial fidelity: $old_fidelity"

        # Obtain specified time for max fidelity
        specified_time_old=$(tail -n +2 /home/hgjones9/quantum_control/max_fidelity_time_$i.txt | head -n 1) 
        echo "Initial time: $specified_time_old"

        # Initialise variables for grad ascent
        ...
        do 
            ...
            if [ "$(echo "$iteration" == 0 | bc)" -eq 1 ]; then
                ...
                python3 /home/hgjones9/quantum_control/initial_genome_adjuster.py "$user_input" "$i"

                # Specify output file of adjusted genomes
                output_file='/home/hgjones9/quantum_control/initial_adjusted_genomes_'$i'.txt'
                ...
                python3 /home/hgjones9/quantum_control/extract_initial_couplings.py "$i"

                # Rename initial_couplings.txt to old_couplings.txt and backup
                mv initial_couplings_$i.txt old_couplings_$i.txt
                cp old_couplings_$i.txt old_couplings_backup_$i.txt
                ...
                python3 /home/hgjones9/quantum_control/coupling_gradient.py "$i"

                echo "Stepsize: $stepsize"

                # Calculate new couplings by gradient ascent
                python3 /home/hgjones9/quantum_control/update_genome.py "$stepsize" "$i"
            
                # Specify output file location of new genome
                new_genome_output='/home/hgjones9/quantum_control/new_genome_'$i'.txt'  

                # Extract new genome from new_genome.txt
                new_genome=$(<"$new_genome_output")
                echo "New genome (B): $new_genome"
                ...
                echo "@$specified_time_old$i_f$new_genome$pos_directive"
                ...
                echo "@${time_minus}${i_f}${new_genome}${pos_directive}"
                /home/hgjones9/spinchain/bin/spinnet "@${time_plus}${i_f}${new_genome}${pos_directive}"
                echo "@${time_plus}${i_f}${new_genome}${pos_directive}"

                # Perform temporal gradient and obtain new fidelity and specified time
                python3 /home/hgjones9/quantum_control/fidelity_time_gradient.py "$i"
                ...
                fidelity=$(head -n 1 /home/hgjones9/quantum_control/new_fidelity_$i.txt) 
                cp new_fidelity_$i.txt new_fidelity_backup_$i.txt
                ...
                specified_time=$(head -n 1 /home/hgjones9/quantum_control/new_time_$i.txt) 
                cp new_time_$i.txt new_time_backup_$i.txt
                ...
                python3 /home/hgjones9/quantum_control/genome_adjuster.py $i

                # Specify output file of adjusted genomes
                adjusted_genomes_out='/home/hgjones9/quantum_control/adjusted_genomes_'$i'.txt'
                ...
                new_genome_output='/home/hgjones9/quantum_control/new_genome_'$i'.txt'   # Specify output file location of new genome
                ...
                echo "@$specified_time$i_f$new_genome$pos_directive"
            ...
            if [ "$(echo "$fidelity_diff < 0" | bc)" -eq 1 ]; then
                ...
                cp new_genome_backup_$i.txt new_genome_$i.txt # Revert to previous genome
                cp old_couplings_backup_$i.txt old_couplings_$i.txt # Revert to previous couplings
                cp new_time_backup_$i.txt new_time_$i.txt

                echo "Fidelity not updated, sticking to previous value of: $fidelity"
                echo "Sticking to previous time: $specified_time"
                
                # Specify output file location of new genome
                genome_output='/home/hgjones9/quantum_control/new_genome_'$i'.txt'
        ...
        echo "${iteration_lst[@]}" > iteration_lst_$i.txt
        echo "${fidelity_lst[@]}" > fidelity_lst_$i.txt
...
#     # Run gradient ascent without user input genome
